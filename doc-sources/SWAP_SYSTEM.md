...
3. [Unified Panel Architecture](#unified-panel-architecture)
4. [Khalani / HyperStream Integration](#khalani--hyperstream-integration)
5. [Uniswap Integration](#uniswap-integration)
6. [Relay.link Integration](#relaylink-integration)
7. [Bebop Integration](#bebop-integration)
8. [Partner Fee System](#partner-fee-system)
9. [Gas Validation System](#gas-validation-system)
...
SuperSafe Wallet integrates **four major swap providers**:

- **Uniswap**: Same-chain DEX swaps through UniswapX and Classic routing.
- **Relay.link**: Cross-chain swap + bridge routes.
- **Khalani / HyperStream**: Cross-chain intent routes with order lifecycle tracking.
- **Bebop**: Gasless same-chain swaps with MEV protection.
...
- **Khalani / HyperStream Integration**: Cross-chain intent routes with quote expiry and lifecycle polling.
- **Gasless Swaps**: Same-chain gasless swaps via Bebop Permit2 orders.
- **MEV Protection**: Protected same-chain execution via Bebop.
- **Shared Safety Controls**: Slippage, price deviation alerts, and gas validation across providers.
- **Background Signing Boundary**: Swap execution that requires keys is delegated to the background service worker.
...
SuperSafe Wallet implements a unified panel architecture for swap providers, keeping shared UI state in `Swap.jsx` and provider-specific logic inside independent panels.
...
| Panel | Responsibility | Secure Boundary |
|-------|----------------|-----------------|
| `UniswapSwapPanel.jsx` | UniswapX and Classic same-chain routing | Background proxy / adapter |
...
SuperSafe integrates the official Uniswap Routing API, Uniswap Universal Router, and UniswapX Dutch Order Protocol for same-chain DEX swaps.
...
SuperSafe implements gas validation for transaction-backed swap actions:
...
For complete gas validation documentation, see [GAS_VALIDATION_SYSTEM.md](./GAS_VALIDATION_SYSTEM.md).
...
- Khalani / HyperStream cross-chain intent routes.
- Four-provider architecture: Uniswap, Relay.link, Khalani / HyperStream, and Bebop.
